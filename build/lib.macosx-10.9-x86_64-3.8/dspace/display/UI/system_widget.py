...
class DisplaySystem(object):
    ...
    def update_display(self):
        ...
        if controller.ds.should_consider_mass_balances is True:
            n = controller.ds.number_of_metabolic_blocks
            s = "This system has <b>{}</b> metabolic blocks. The mass balances for each block are the following.".format(n)
            for i in range(n):
                s += "<b> Block {}: </b>".format(i+1)
                s += ", ".join([controller.ds.fin_at_index(i), controller.ds.fout_at_index(i)])
                s += "; " if i != n-1 else ""
            s += "."
            self.metabolic_blocks.value = s
