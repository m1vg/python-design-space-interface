    ...
    def stoichiometric_connectivity_matrices(self):
        ...
        for n, i in enumerate(eqs):
            ...
            for ii in range(DSExpressionNumberOfTerms(rhs._swigwrapper)):
                ...
                S[metabolic_pools.index(pool), rxns.index(flux.replace('-', ''))] = + coeff
                N[metabolic_pools.index(pool), rxns.index(flux.replace('-', ''))] = 1

        return [S, N, metabolic_pools, rxns]

    def generate_mass_balances(self):

        # 1. We first get the stoichiometric and connectivity matrices.
        S, N, metabolic_pools, rxns = self.stoichiometric_connectivity_matrices()

        #2. we now identify metabolic groups
        assigned = []
        groups = []
        for pool_i in metabolic_pools:
            if pool_i in assigned:
                continue
            index1 = metabolic_pools.index(pool_i)
            group_i = []
            for pool_ii in metabolic_pools:
                if pool_ii == pool_i:
                    continue
                index2 = metabolic_pools.index(pool_ii)
                if sum(N[index1] * N[index2]) != 0:
                    if len(group_i) == 0:
                        for i in [pool_i, pool_ii]:
                            group_i.append(metabolic_pools.index(i))
                            assigned.append(i)
                    if pool_ii not in assigned:
                        group_i.append(metabolic_pools.index(pool_ii))
                        assigned.append(pool_ii)
            if len(group_i) != 0:
