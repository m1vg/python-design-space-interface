...
class DisplayCase(object):
    ...
    def __init__(self, controller, case_id, by_signature=False, pvals=None, constraints=None, subtitle=''):
        ...
        setattr(self, 'case_adjusted_stoichiometry', controller.ds(case_id,
                                                                   by_signature=by_signature,
                                                                   constraints=constraints))
    ...
    def update_equations(self):
        controller = self.controller
        case = self.case
        case_adjusted = self.case_adjusted_stoichiometry
        if case_adjusted.is_cyclical is False:
            case_adjusted.adjust_codominant_stoichiometry()
        ...
        equations = Latex()
        adjusted_equations = Latex()
        if self.dynamic_only is False:
            equations.value = case.equations._repr_latex_()
            adjusted_equations.value = case_adjusted.equations._repr_latex_()
        else:
            ssys = case.ssystem.remove_algebraic_constraints()
            ssys_adjusted = case_adjusted.ssystem.remove_algebraic_constraints()
            equations.value = ssys.equations._repr_latex_()
            adjusted_equations.value = ssys_adjusted.equations._repr_latex_()
        solution = Latex()
        conditions = Latex() 
        boundaries = Latex()
        adjusted_solution = Latex()
        collapsed_equations = Latex()
        cyclical_description = Latex()
        codominant_description = Latex()
        if self.log_coordinates is True:
            ...
            adjusted_solution_data = case_adjusted.ssystem.solution_log
            ...
            adjusted_solution_data = case_adjusted.ssystem.solution
        ...
        if solution_data is not None:
            adjusted_solution.value = adjusted_solution_data._repr_latex_()
        else:
            adjusted_solution.value = 'Adjusting Stoichiometry Failed'
        ...
        if self.case.ssystem.should_adjust_codominant_stoichiometry is True and case.number_of_mass_balances != 0:
            n = case.number_of_mass_balances
            s = "This network has {} metabolic blocks. The dominant exchange fluxes for each block are: ".format(n)
            fin_list = [case.dominant_fin_at_index(i) for i in range(case.number_of_mass_balances)]
            fout_list = [case.dominant_fout_at_index(i) for i in range(case.number_of_mass_balances)]
            for j in range(n):
                ratio = "{}/({})".format(fin_list[j], fout_list[j])
                ratio_simplified = DSExpressionByParsingStringSimplifyExpressionAsString(ratio)
                s += "Fin$_{}$ = {}; Fout$_{}$ = {}; ".format(j+1, fin_list[j], j+1, fout_list[j])
                s += "Fin$_{}$/Fout$_{}$ = {}. ".format(j+1, j+1, ratio_simplified)
            codominant_description.value = s
        ...
        self.equations.children = [check_box,
                                   ...
                                   boundaries] + \
                                  ([ HTML(value='<br>'),
                                     HTML(value='<b>Adjusted Equations:</b><br>'),
                                     adjusted_equations,
                                     HTML(value='<br>'),
                                     HTML(value='<b>Adjusted Solution:</b><br>'),
                                     adjusted_solution,
                                     HTML(value='<br>'),
                                     codominant_description,

                                     ] if self.case.ssystem.should_adjust_codominant_stoichiometry is True else []) + \
                                  ([
                                    HTML(value='<br>'),
