...
class FullSystem(object):
    ...
    def fullsystem_widget(self):
        ...
        y_min = FloatText(description='log Y-Min', value=-2, visible=False)
        y_max = FloatText(description='log Y-Max', value=2, visible=False)
        ...
        self.s_system_steady_states = Checkbox(value=False, description='Show S-System Approximation')
    ...
    def update_field(self, name, value):
        ...
        if value == 'Time Courses':
            ...
            self.caption.value = 'Concentration of ' + self.ylabel.value + ' is plotted as a function of ' \
                                 + self.xlabel.value + '.'
    ...
    def titrations_case(self, b):
        ...
        if self.backwards.value is True:
            ...
            zuru = results_dic_back[self.ylabel.value]

            # End of backwards routine

        # lets generate hin vector
        hin = results_dic[self.ylabel.value]
        ...
        # ax.set_ylabel(r'$Log_{10}($'+self.ylabel.value+')')
        ax.set_ylabel(self.process_label_string(self.ylabel.value))
        ax.plot(np.log10(target_x_parameter), self.process_results_vector(hin), 'blue', linewidth=1)

        if self.backwards.value is True:
            ax.plot(np.log10(target_x_parameter_reversed), self.process_results_vector(zuru), 'orange', linewidth=1)

        ax.ticklabel_format(useOffset=False)

        # Run routine to generate titration plot from the Toolbox. It is the method draw_1D_ss_function of the design
        # space object.
        # function = str('log('+self.ylabel.value+')')
        fun = self.process_label_string(self.ylabel.value, function=True)
        rangex = [self.x_min.value, self.x_max.value]
        if self.s_system_steady_states.value is True:
        ...
        # if max(np.log10(hin)) > 0.03 or max(np.log10(hin)) < -0.03:
        #     ax.set_ylim([min(np.log10(hin))*1.2, max(np.log10(hin))*1.2])
        # else:
        #     ax.set_ylim([min(np.log10(hin))*1.2, max(np.log10(hin))+0.001])
        ax.set_ylabel(self.process_label_string(self.ylabel.value))
